...
def get_transpiler(baseDecomposition = True, placeAndRoute = True, optimization = True, nativeDecomposition = True):
    def transpile(tape : QuantumTape):
        ...
        with qml.QueuingManager.stop_recording():
            ...
            if placeAndRoute: optimized_tape = step3.physical_placement(optimized_tape)
            if placeAndRoute: optimized_tape = step4.swap_routing(optimized_tape)
            if optimization: optimized_tape = step2.optimize(optimized_tape)
